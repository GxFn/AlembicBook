...
Visible title at top: "知识进化全链路数据流"

Purpose:
Show the end-to-end path from file events and Agent behavior to lifecycle changes.

Main layout:
- Four horizontal bands: 触发层, 信号层, 决策层, 落地层.
- Trigger band: FileChangeHandler, Guard result, Search usage, Agent submit.
- Signal band: SignalBus with 12 signal types.
- Decision band: RelevanceAuditor, EvolutionGateway, RecipeProductionGateway.
- Landing band: staging, active, proposal, deprecated, delivery refresh.

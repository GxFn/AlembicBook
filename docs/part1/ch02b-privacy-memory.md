...
>
> **相关章节**：记忆的工程实现见 [ch15 工具与记忆系统](../part5/ch15-tools-memory)，信号与知识治理见 [ch12 Panorama · Signal](../part4/ch12-metabolism)，交付通道见 [ch17 MCP 协议](../part6/ch17-mcp-delivery)
...
时间尺度：持续积累，SignalTraceWriter 留痕，重扫与 Evolution 治理流程消费
类型：guard · search · usage · intent · lifecycle
迁移方式：文件复制
```

行为信号是知识进化的燃料。SignalCollector、重扫计划和 Evolution 服务从中发现：哪些 Recipe 被频繁搜索（说明它有价值），哪些 Guard 规则一直没命中（可能过时了），哪些知识在 Prime 时被检索但 Agent 最终没用（可能需要修订）。

信号的价值在于它是**去中心化的观测**——不是平台在观察你，而是你的项目在观察自己。数据不离开本地，消费者是你本地的信号、重扫与进化治理链路。
...
第四层（会话上下文）
  ...
  ↓
第二层（行为信号）
  │ 重扫 / Evolution 消费：衰退检测 · 冗余分析 · 进化提案
  ↓
第三层（Agent 记忆）
...
一条知识的典型旅程：Agent 在一次会话中发现"这个项目的错误处理遵循特定模式"（第四层）。会话结束后，这个发现作为意图信号持久化（第二层）。多次会话积累后，模式被确认为项目事实（第三层）。当重扫或 Evolution 审计检测到足够的信号支撑时，生成一条 Evolution Proposal，经人工审核后成为正式 Recipe（第一层）。
...
Agent 的行为记忆反哺知识库——每次交互产生的信号被本地信号、重扫与 Evolution 链路持续消费：

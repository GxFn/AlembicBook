...
::: warning 阅读指引
本章是 [ch11 Search 混合检索](./ch11-search) 的深入版——覆盖 HNSW 图算法、SQ8 量化数学、二进制持久化格式等底层实现。如果你只需要理解搜索系统的全貌，可以跳过本章，直接进入 [ch12 Panorama · Signal · 代谢](./ch12-metabolism)。
:::
...
**几何直觉**：余弦相似度本质上衡量的是**投影**而不是垂直距离。$\mathbf{a} \cdot \mathbf{b} = \lVert\mathbf{a}\rVert \times \lVert\mathbf{b}\rVert \times \cos\theta$，除以两者模长后得到纯粹的 $\cos\theta$——只和方向夹角有关，和向量长度无关。这正是它适合文本 embedding 的原因：embedding 模型把语义信息编码在向量的**方向**上，而长度差异只反映文本长短或模型的数值波动，不携带语义。两篇语义相近的文档，向量指向接近同一方向（夹角小 → 投影大 → 余弦距离小）。

**计算量**：看似简单的循环，768 维意味着每次距离计算需要 768 × 3 = 2304 次乘法和同等次数的加法，总计约 4600 次浮点运算。如果暴力扫描 5000 条向量，就是 2300 万次浮点运算。这正是 HNSW 存在的意义——通过图导航把需要计算距离的节点从 $O(N)$ 减少到 $O(\log N)$，5000 条中通常只需对 ~150 个节点计算距离。SQ8 量化则在此基础上进一步用整数运算替代浮点运算，整数循环在 V8 JIT 编译下比浮点快得多。
...
| 模型 | 维度 | 大小 | 单次延迟 |
|:---|:---|:---|:---|
| `all-MiniLM-L6-v2` | 384 | 80 MB | ~5ms |
| `bge-small-en-v1.5` | 384 | 130 MB | ~8ms |
| `nomic-embed-text-v1.5` | 768 | 550 MB | ~20ms |

这些模型在短文本（< 20 个词的查询）上的表现接近大模型 embedding 的 85-90%，完全满足知识库搜索的精度需求。需要注意的是维度必须全系统统一——如果改用 384 维的本地模型，需要全量重建索引。

还有一些**完全不需要神经网络**的方案：

- **TF-IDF 向量**：统计词频生成稀疏向量，精确关键词匹配效果好，但不理解同义词（"汽车"和"车辆"被视为不同的词）
